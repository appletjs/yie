...
var Transition = _interopDefault(require('preact-transition'));
var preactRouter = require('preact-router');
 ...
 * @return {function(*): *}
 */
function makeAccessor(str, separator) {
  ...
  switch (typeof value) {
    ...
    case 'object':
      ...
      if (Array.isArray(value)) {
        parsed = value.map(this.explode, this).reduce(function (x, a) { return x.concat(a); }, []);
        break;
      }

      // {className: check, ...}
      parsed = Object.keys(value).map(function (key) { return value[key] && key; });

  }

  if (parsed.length) {
    return parsed.map(function (c) { return c.trim(); }).filter(Boolean).filter(function check(className) {
...
function util(e) {
  return iconSources[e] || function () {
    throw new Error('Cannot find module \'' + e + '\'.');
  }();
}

function keys() {
  return Object.keys(iconSources);
}

function generateHyperSvg(h$$1, name, attrs) {
  if ( attrs === void 0 ) attrs = {};

  var use = h$$1('use', { 'xlink:href': ("#ic_" + name + "_24px") });
  return h$$1('svg', attrs, use);
}

function generateHyperIcon(h$$1, name, attrs) {
  if ( attrs === void 0 ) attrs = {};

  attrs.className = [attrs.className, 'svg-icon'].filter(Boolean).join(' ');
  attrs.children = [generateHyperSvg(h$$1, name)];
  return h$$1('div', attrs);
}

(function install() {
  var ICON_SOURCE_ELEMENT_ID = 'svg-icon-sources';
  if (document.getElementById(ICON_SOURCE_ELEMENT_ID)) { return; }
  var e = document.createElement('div');
  e.id = ICON_SOURCE_ELEMENT_ID;
  e.style.display = 'none';
  keys().forEach(function (t) { return e.innerHTML += util(t); });
  document.body.insertBefore(e, document.body.firstChild);
})();

var InkClassName = 'ui-ripple-ink';

function getHolder(event) {
  var holder = event.target || event.currentTarget;
  return holder && !holder.classList.contains(InkClassName) ? holder.querySelector('.' + InkClassName) : holder;
}
...
var RippleInk = (function (Component$$1) {
  function RippleInk () {
    Component$$1.apply(this, arguments);
  }

  if ( Component$$1 ) RippleInk.__proto__ = Component$$1;
  RippleInk.prototype = Object.create( Component$$1 && Component$$1.prototype );
  RippleInk.prototype.constructor = RippleInk;

  RippleInk.prototype.bindRipple = function bindRipple () {
    var ref = this.props;
  ...
  RippleInk.prototype.destroyRipple = function destroyRipple () {
  ...
  RippleInk.prototype.componentDidMount = function componentDidMount () {
    setTimeout(this.bindRipple.bind(this));
  };

  RippleInk.prototype.componentWillUnmount = function componentWillUnmount () {
    this.destroyRipple();
  };

  RippleInk.prototype.componentWillReceiveProps = function componentWillReceiveProps () {
    this.bindRipple();
  };

  RippleInk.prototype.render = function render () {
    return preact.h('div', { className: InkClassName });
  };

  return RippleInk;
}(preact.Component));

var Ripple = (function (Component$$1) {
  function Ripple () {
    Component$$1.apply(this, arguments);
  }

  if ( Component$$1 ) Ripple.__proto__ = Component$$1;
  Ripple.prototype = Object.create( Component$$1 && Component$$1.prototype );
  Ripple.prototype.constructor = Ripple;

  Ripple.prototype.render = function render (props) {
    var this$1 = this;

    var attrs = without(props, ['ref', 'component']);
    var children = attrs.children = (props.children || []).slice();
    var ref = props.ref;
    var component = props.component; if ( component === void 0 ) component = 'div';

    var classList = TokenList.from(props.className);
    classList.add('ui-ripple');
    attrs.className = classList.toString();

    attrs.ref = function (node) {
      this$1.containerDOM = node;
      ref && ref(node);
    };

    children.push(preact.h(RippleInk, {
      center: props.centerRipple,
      trigger: function () { return this$1.containerDOM; }
    }));

    return preact.h(component, attrs);
  };

  return Ripple;
}(preact.Component));

var MenuItem = (function (Component$$1) {
  function MenuItem () {
    Component$$1.apply(this, arguments);
  }

  if ( Component$$1 ) MenuItem.__proto__ = Component$$1;
  MenuItem.prototype = Object.create( Component$$1 && Component$$1.prototype );
  MenuItem.prototype.constructor = MenuItem;

  MenuItem.prototype.render = function render (props) {
    var text = props.text;
    var icon = props.icon;
    ...
    var attrs = without(props, ['text', 'icon', 'selected', 'disabled', 'component']);
    ...
    var childVNodes = [];
    if (icon) {
      var textVNode = text ? preact.h('span', {}, text) : '';
      childVNodes.push(generateHyperSvg(preact.h, icon, { className: 'ui-menu-item-icon', focusable: 'false' }));
      childVNodes.push(preact.h(
        'div',
        { className: 'ui-menu-item-text' },
        textVNode
      ));
    } else if (text) {
    ...
    var component = props.component; if ( component === void 0 ) component = 'li';

    if (!props.disabled) {
      attrs.component = component;
      return preact.h(Ripple, attrs);
    }
...
var hooks = ['onBeforeOpen', 'onOpen', 'onAfterOpen', 'onBeforeClose', 'onClose', 'onAfterClose'];

var proxy = ['onBeforeEnter', 'onEnter', 'onAfterEnter', 'onBeforeLeave', 'onLeave', 'onAfterLeave'];

var excludeProps = hooks.concat(['component', 'raised', 'open', 'origin', 'items']);
...
var Menu = (function (Component$$1) {
  ...
  Menu.prototype.render = function render (props) {
    ...
    var style = attrs.style ? attrs.style + ';' : '';
    ...
    if (Array.isArray(props.items)) {
      attrs.children = props.items.map(function (item) {
        if (item == null || item === false) { return null; }
        if (item === true) { return preact.h('li', { className: 'ui-menu-divider' }); }
        if (typeof item === 'function') { return item; } // MenuItem or generator
        if (typeof item === 'string') { return preact.h(MenuItem, { text: item }); } // todo click 事件
        if (item.icon) { classList.add('has-icon'); }
        return preact.h(MenuItem, item); // 包括 onClick 事件等
      });
    }

    attrs.className = classList.toString();

    return preact.h(Transition, attrs);
  };

  return Menu;
}(preact.Component));

var Icon = (function (Component$$1) {
  function Icon () {
    Component$$1.apply(this, arguments);
  }

  if ( Component$$1 ) Icon.__proto__ = Component$$1;
  Icon.prototype = Object.create( Component$$1 && Component$$1.prototype );
  Icon.prototype.constructor = Icon;

  Icon.prototype.render = function render (props) {
    var classList = TokenList.from(props.className);
    props.color && classList.add(props.color);
    classList.add(props.size || 'medium');

    var attrs = without(props, ['name', 'size', 'color']);
    attrs.className = classList.toString();

    return generateHyperIcon(preact.h, props.name, attrs);
  };

  return Icon;
}(preact.Component));

var LoadingIndicator = (function (Component$$1) {
  function LoadingIndicator () {
    Component$$1.apply(this, arguments);
  }

  if ( Component$$1 ) LoadingIndicator.__proto__ = Component$$1;
  LoadingIndicator.prototype = Object.create( Component$$1 && Component$$1.prototype );
  LoadingIndicator.prototype.constructor = LoadingIndicator;

  LoadingIndicator.prototype.render = function render (props) {
    var classList = TokenList.from(props.className);
    ...
    var attrs = without(props, LoadingIndicator.attributes);
    attrs.className = classList.toString();
    attrs.children = [preact.h('div', { className: 'animation' })];
    return preact.h('div', attrs);
  };

  return LoadingIndicator;
}(preact.Component));
LoadingIndicator.attributes = ['color', 'inline', 'overlay', 'size'];

var excludes = ['size', 'color', 'type', 'disabled', 'leftIcon', 'rightIcon', 'loading', 'value', 'component', 'block', 'round', 'onClick'];
...
var Button = (function (Component$$1) {
  ...
  Button.prototype.render = function render (props, state) {
    ...
    var component = props.component; if ( component === void 0 ) component = 'button';
    var attrs = without(props, excludes);

    attrs.onClick = function (e) { return this$1.handleClick(e); };
    attrs.component = component;
    ...
    var c = type === 'flat' ? color : '';
    if (loading === 'inline') {
      content.push(preact.h(LoadingIndicator, { size: 'small', color: c, inline: true }));
    } else if (props.loading) {
      children.push(preact.h(LoadingIndicator, { color: c, overlay: true }));
    }

    if (props.leftIcon && loading !== 'inline') {
      content.push(preact.h(Icon, { className: 'ui-button-icon left', size: size, name: props.leftIcon }));
    }

    if (props.value) {
      content.push(preact.h(
        'div',
        { className: 'ui-button-value' },
        props.value
      ));
    }

    if (props.rightIcon) {
      content.push(preact.h(Icon, { className: 'ui-button-icon right', size: size, name: props.rightIcon }));
    }

    children.push(preact.h(
      'div',
...
var IconButton = (function (Component$$1) {
  function IconButton () {
    Component$$1.apply(this, arguments);
  }

  if ( Component$$1 ) IconButton.__proto__ = Component$$1;
  IconButton.prototype = Object.create( Component$$1 && Component$$1.prototype );
  IconButton.prototype.constructor = IconButton;

  IconButton.prototype.render = function render (props) {
    var attrs = without(props, ['value', 'rightIcon', 'loading']);
...
var Text = (function (Component$$1) {
  ...
  Text.prototype.render = function render (props) {
    // 没有内容不渲染到DOM上
    if (props.value === undefined && props.children.length === 0) {
      return null;
    }

    var attrs = without(props, ['lines', 'ellipsis', 'value']);

    if (props.value !== undefined) {
      attrs.children = [String(props.value).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
      // .replace(/ /g, '&nbsp;')
      .replace(/\'/g, '&#39;').replace(/\"/g, '&quot;')];
    }

    var styleMap = StyleMap.from(props.style);
...
var Toolbar = (function (Component$$1) {
  ...
  Toolbar.prototype.componentDidMount = function componentDidMount () {
    ...
    this._clickOutsideListener = function (e) {
      if (this$1.animating || !this$1.menuAction) { return; }
      if (this$1.menuAction.contains(e.target)) { return; }
      if (!this$1.state.open) { return; }
      this$1.setState({ open: false });
    };
  ...
  Toolbar.prototype.render = function render (props, state) {
    var this$1 = this;

    var attrs = without(props, Toolbar.includeProps);
    var children = attrs.children = [];

    // Navigation
    if (props.navIcon) {
      children.push(preact.h(IconButton, {
    ...
    // Title
    if (props.title) {
      children.push(preact.h(Text, { className: 'ui-toolbar-title', value: props.title }));
    } else {
      children.push(preact.h(
        'div',
    ...
    if (Array.isArray(props.actions)) {
      var onActionClick = createHandle(props.onActionClick);
      var onMenuClick = createHandle(props.onMenuClick || props.onActionClick);
      var max = typeof props.maxActionCount === 'number' ? props.maxActionCount : 2;
      ...
      props.actions.slice(0, max).forEach(function (act) {
        if (act.nodeName) {
          ...
          children.push(preact.h(IconButton, {
            ...
            size: 'big',
            icon: act.icon,
            onClick: function (e) { return (act.onClick || onActionClick)(e, act); },
            type: 'flat',
            __center__: true,
            round: true }));
        }
      ...
      menus = props.actions.slice(max).map(function (menu) {
        ...
        return preact.h(MenuItem, { text: menu.text, icon: menu.icon, onClick: click });
      });
    ...
    if (menus && menus.length) {
      ...
      var refMenuIcon = function (node) { return this$1.menuAction = node; };
      var onAnimateStart = function () { return this$1.animating = true; };
      var onAnimationStop = function () { return this$1.animating = false; };

      children.push(preact.h(
        'div',
        { className: 'ui-toolbar-menu', ref: refMenuIcon },
        preact.h(IconButton, { size: 'big', icon: 'more_vert', type: 'flat', onClick: onToggleMenu, round: true }),
        preact.h(
          Menu,
          {
...
Toolbar.includeProps = ['title', 'navIcon', 'onNavClick', 'actions', 'maxActionCount', 'onActionClick', 'onMenuClick', 'shadowed', 'color', 'static'];

var AppActivity = (function (Component$$1) {
  ...
  if ( Component$$1 ) AppActivity.__proto__ = Component$$1;
  AppActivity.prototype = Object.create( Component$$1 && Component$$1.prototype );
  AppActivity.prototype.constructor = AppActivity;

  AppActivity.prototype.onToolbarChange = function onToolbarChange () {
    var this$1 = this;

    var ref = this.context;
    var onToolbarChange = ref.onToolbarChange;
    if (typeof onToolbarChange !== 'function') { return; }
    if (this.willUpdateToolbar) { return; }
    this.willUpdateToolbar = true;
    var states = Object.assign({}, this.toolbarProps);
    onToolbarChange(states, function () {
      this$1.willUpdateToolbar = false;
    });
  };

  AppActivity.prototype.componentWillReceiveProps = function componentWillReceiveProps (nextProps) {
    this.renderTimes++;

    var states = this.toolbarProps;
    var toolbarProps = {};
    var changed = false;

    Toolbar.includeProps.forEach(function (key) {
      if (nextProps.hasOwnProperty(key) && states.hasOwnProperty(key) && states[key] !== nextProps[key]) {
        changed = true;
  ...
  AppActivity.prototype.componentDidMount = function componentDidMount () {
    this.onToolbarChange();
    this.renderTimes++;
  };

  AppActivity.prototype.render = function render (props) {
    // Attributes
    var attrs = without(props, Toolbar.includeProps.concat('children'));
    var children = attrs.children = [];

    attrs.className = TokenList.from(props.className).add('app-activity').toString();
    ...
    if (navIcon || title || actions) {
      children.push(preact.h("div", { className: "app-activity-toolbar-holder" }));
    }

    // Children
    if (props.children) {
      children.push.apply(children, props.children);
    }
...
var AppLayout = (function (Component$$1) {
  ...
  if ( Component$$1 ) AppLayout.__proto__ = Component$$1;
  AppLayout.prototype = Object.create( Component$$1 && Component$$1.prototype );
  AppLayout.prototype.constructor = AppLayout;

  AppLayout.prototype.getChildContext = function getChildContext () {
    var ref = this;
    var onToolbarChange = ref.onToolbarChange;
    var showToolbar = ref.showToolbar;
    var hideToolbar = ref.hideToolbar;
    return { onToolbarChange: onToolbarChange, showToolbar: showToolbar, hideToolbar: hideToolbar };
  };

  AppLayout.prototype.showToolbar = function showToolbar () {
    if (this.toolbarAnimating) { return; }
    if (this.state.toolbarMode === 'in') { return; }
    this.setState({ toolbarMode: 'in' });
  };

  AppLayout.prototype.hideToolbar = function hideToolbar () {
    if (this.toolbarAnimating) { return; }
    if (this.state.toolbarMode === 'out') { return; }
    this.setState({ toolbarMode: 'out' });
  };

  AppLayout.prototype.setToolbar = function setToolbar (states, cb) {
    var this$1 = this;

    if (this.toolbarAnimating) {
      nextTick(function () { return this$1.setToolbar(states, cb); });
    } else {
      var toolbar = Object.assign({}, defaultToolbarProps, states);
      var toolbarMode = toolbar.title || toolbar.navIcon || toolbar.actions && toolbar.actions.length ? 'in' : 'out';
      this.setState({ toolbar: toolbar, toolbarMode: toolbarMode }, function () { return cb && cb(); });
    }
  };

  AppLayout.prototype.onToolbarChange = function onToolbarChange (states, cb) {
    var this$1 = this;

    var navIcon = states.navIcon;
    var title = states.title;
    var actions = states.actions;
    var attrs = {};
    ...
    if (states.navIcon === true) {
      ...
      if (!states.onNavClick && this.props.history) {
        attrs.onNavClick = function () {
          this$1.props.history.goBack();
        };
      }
    }
  ...
  AppLayout.prototype.generateToolbar = function generateToolbar () {
    var this$1 = this;

    var attrs = Object.assign({}, this.state.toolbar);
    var start = function () { return this$1.toolbarAnimating = true; };
    var stop = function () { return this$1.toolbarAnimating = false; };
    attrs.tag = Toolbar;
  ...
  AppLayout.prototype.generateBottomNavigation = function generateBottomNavigation () {};

  AppLayout.prototype.render = function render (props, state) {
    return preact.h(
      'div',
      ...
      preact.h(
        preactRouter.Router,
        { history: props.history, onChange: props.onRouterChange },
        props.children
      )
    );
  };
...
var access = makeAccessor('primary,accent,danger,warning,info,success');

var Avatar = (function (Component$$1) {
  function Avatar () {
    Component$$1.apply(this, arguments);
  }

  if ( Component$$1 ) Avatar.__proto__ = Component$$1;
  Avatar.prototype = Object.create( Component$$1 && Component$$1.prototype );
  Avatar.prototype.constructor = Avatar;

  Avatar.prototype.render = function render (props) {
    var classList = TokenList.from(props.className);
    classList.add('ui-avatar');
    if (props.tile) { classList.add('tile'); }

    var styleMap = StyleMap.from(props.style);
    var size = convertSize(props.size || 36);
    styleMap.set('width', size);
    styleMap.set('height', size);

    var color = props.color;
    if (color && access(color)) { classList.add(color); }else if (color) { styleMap.set('background', color); }

    var attrs = without(props, ['tile', 'size', 'color']);
    attrs.className = classList.toString();
    attrs.style = styleMap.toString();
...
var access$1 = makeAccessor('primary,accent,danger,warning,info,success');
var CLASS_NAME = 'ui-bottom-navigation';

var BottomNavigation = (function (Component$$1) {
  ...
  if ( Component$$1 ) BottomNavigation.__proto__ = Component$$1;
  BottomNavigation.prototype = Object.create( Component$$1 && Component$$1.prototype );
  BottomNavigation.prototype.constructor = BottomNavigation;

  BottomNavigation.prototype.componentWillReceiveProps = function componentWillReceiveProps (ref) {
  ...
  BottomNavigation.prototype.proxyItemClick = function proxyItemClick (selected, item) {
    var this$1 = this;

    return function (e) {
      var onItemClick = item.onClick || this$1.props.onItemClick;
      if (selected !== this$1.state.selected) { this$1.setState({ selected: selected }); }
      if (typeof onItemClick !== 'function') { return; }
      onItemClick(e, item.__raw || item);
    };
  };

  BottomNavigation.prototype.ensureItems = function ensureItems () {
    var ref = this.props;
    var items = ref.items;
  ...
  BottomNavigation.prototype.ensureNavItem = function ensureNavItem (index, item, attrs) {
    var ref = this.props;
    var textColor = ref.textColor;
    var selectedTextColor = ref.selectedTextColor;
    var ref$1 = this.state;
    var selected = ref$1.selected;
    var children = [];

    if (item.icon) {
      children.push(preact.h(Icon, {
    ...
    attrs.children.push(preact.h(Ripple, {
  ...
  BottomNavigation.prototype.render = function render (props) {
    var this$1 = this;

    var items = this.ensureItems();
    items = items && items.filter(Boolean);
    if (!items || !items.length) { return null; }

    var backgroundColor = props.backgroundColor;
    var styles = new StyleMap();
    var classList = new TokenList();

    classList.add(CLASS_NAME);
    classList.add(props.type || 'static');

    if (access$1(backgroundColor)) {
    ...
    items.forEach(function (item, index) {
      this$1.ensureNavItem(index, item, attrs);
    });
...
var Tipbox = (function (Component$$1) {
  function Tipbox () {
    Component$$1.apply(this, arguments);
  }

  if ( Component$$1 ) Tipbox.__proto__ = Component$$1;
  Tipbox.prototype = Object.create( Component$$1 && Component$$1.prototype );
  Tipbox.prototype.constructor = Tipbox;

  Tipbox.prototype.generateFooter = function generateFooter (attrs, props) {
    ...
    // 取消按钮
    var cancelValue = props.cancelValue;
    ...
    buttons = buttons.filter(Boolean).reduce(function (btns, btn) {
      if (!btn) { return btns; }
      btns.push(preact.h(Button, getButtonAttributes(btn, onClick)), preact.h('div', { className: 'ui-tipbox-divider' }));
      return btns;
    }, []);
    ...
    attrs.children.push(preact.h(
      'footer',
      { className: 'ui-tipbox-footer' },
      buttons
    ));
  };

  Tipbox.prototype.generateTitle = function generateTitle (attrs, props) {
    var children = [];
    ...
    if (dismiss && isPreactElement(dismiss)) {
      children.push(preact.h(dismiss, { className: 'ui-tipbox-dismiss' }));
    ...
    if (children.length) {
      attrs.children.push(preact.h(
        'header',
        { className: 'ui-tipbox-header' },
        children
      ));
    }
  };

  Tipbox.prototype.generateBody = function generateBody (attrs, prop) {
    if (prop.children && prop.children.length) {
      attrs.children.push(preact.h(
        'div',
        { className: 'ui-tipbox-body' },
        prop.children
      ));
    }
  };

  Tipbox.prototype.render = function render (props) {
    var classList = TokenList.from(props.className).add('ui-tipbox');
    ...
    var attrs = without(props, Tipbox.includeProps);
    attrs.className = classList.toString();
    attrs.children = [];
    ...
    return preact.h(props.component || 'div', attrs);
  };

  return Tipbox;
}(preact.Component));
Tipbox.includeProps = ['okValue', 'cancelValue', 'onOk', 'onCancel', 'buttons', 'title', 'color', 'onButtonClick', 'dismiss', 'onDismiss'];

var dialogProps = ['display', 'dimmed', 'docked', 'children', 'className', 'onShow', 'onHide'];
...
var Dialog = (function (Component$$1) {
  ...
  if ( Component$$1 ) Dialog.__proto__ = Component$$1;
  Dialog.prototype = Object.create( Component$$1 && Component$$1.prototype );
  Dialog.prototype.constructor = Dialog;

  Dialog.prototype.componentWillReceiveProps = function componentWillReceiveProps (props) {
    var mode = props.display ? 'in' : 'out';
    if (mode !== this.state.mode) {
      this.setState({ mode: mode });
    }
  };

  Dialog.prototype.render = function render (props, state) {
    var attrs = without(props, dialogProps.concat(Tipbox.includeProps));
    var classList = TokenList.from(props.className);
    ...
    var onHide = createEventListener(props.onHide);
    var onShow = createEventListener(props.onShow);

    var tipBoxAttrs = without(props, ['className'].concat(dialogProps));
    ...
    var tipTbox = preact.h(Tipbox, tipBoxAttrs);
    ...
    return preact.h(Transition, attrs, tipTbox);
  };

  return Dialog;
}(preact.Component));

var invalidTypes = ['file', 'checkbox', 'radio', 'hidden', 'image', 'button', 'reset', 'submit'];

var inputProps = [
// common
'type', 'value', 'alt', 'name', 'placeholder', 'id', 'maxLength', 'minLength', 'max', 'min', 'autocomplete', 'autofocus', 'pattern', 'formnovalidate', 'form', 'list', 'disabled', 'readonly', 'required', 'onChange', 'onInput', 'onFocus', 'onBlur', 'onKeyUp', 'onKeyDown', 'onKeyPress', 'rows'];

var excludeProps$1 = inputProps.concat('loading', 'leftIcon', 'rightIcon', 'status', 'size', 'flat', 'multiple', 'block');
...
var Input = (function (Component$$1) {
  ...
  Input.prototype.render = function render (props, state) {
    ...
    var isBig = props.size === 'big';
    var iconSize = isBig ? 'big' : undefined;
    var loadSize = isBig ? undefined : 'small';
    ...
    if (props.loading === 'left') {
      content.push(preact.h(LoadingIndicator, { className: 'ui-input-left-loading', size: loadSize, inline: true }));
    } else if (props.leftIcon) {
      content.push(preact.h(Icon, { className: 'ui-input-left-icon', size: iconSize, name: props.leftIcon }));
    }

    // 输入框左边自定义
    if (slots.left && Array.isArray(slots.left)) {
      content.push.apply(content, slots.left);
    } else if (slots.left) {
    ...
    // 输入框
    {
      ...
      if (!attrs$1.type) {
        attrs$1.type = 'text';
      } else if (invalidTypes[attrs$1.type]) {
        throw new Error('Invalid Type: ' + attrs$1.type + '\n  allowed ' + invalidTypes.join(','));
      }
      ...
      attrs$1.onFocus = function (e) {
        if (onFocus) { onFocus(e); }
        if (state.focused) { return; } // primary
        this$1.setState({ focused: true });
      };

      attrs$1.onBlur = function (e) {
        if (onBlur) { onBlur(e); }
        if (!state.focused) { return; } // primary
        this$1.setState({ focused: false });
      };

      // 收集 Input 节点
      attrs$1.ref = function (node) {
        this$1.INPUT = node;
      };

      attrs$1.className = 'ui-input-field';

      content.push(preact.h(props.multiple ? 'textarea' : 'input', attrs$1));
    }

    // 输入框右边自定义
    if (slots.right && Array.isArray(slots.right)) {
      content.push.apply(content, slots.right);
    } else if (slots.right) {
    ...
    if (props.loading === 'right') {
      content.push(preact.h(LoadingIndicator, { className: 'ui-input-right-loading', size: loadSize, inline: true }));
    } else if (props.rightIcon) {
      content.push(preact.h(Icon, { className: 'ui-input-right-icon', size: iconSize, name: props.rightIcon }));
    }

    // 聚焦动画
    content.push(preact.h('div', { className: 'ui-input-animation' }));
    ...
    var handleFocus = function (e) {
      ...
      if (!this$1.INPUT) { return; } // 还没有挂在到DOM上
      this$1.INPUT.focus();
    };
    children.push(preact.h(
      'div',
      { className: 'ui-input-content', onClick: handleFocus },
      content
    ));
    ...
    var attrs = without(props, excludeProps$1);
    attrs.className = classList.toString();
    attrs.children = children;
...
var Paper = (function (Component$$1) {
  function Paper () {
    Component$$1.apply(this, arguments);
  }

  if ( Component$$1 ) Paper.__proto__ = Component$$1;
  Paper.prototype = Object.create( Component$$1 && Component$$1.prototype );
  Paper.prototype.constructor = Paper;

  Paper.prototype.render = function render (props) {
    var attrs = without(props, Paper.attributes);
    var classes = TokenList.from(props.className);
    var component = props.component; if ( component === void 0 ) component = 'div';
    var elevation = props.elevation; if ( elevation === void 0 ) elevation = 3;
...
Paper.attributes = ['component', 'elevation'];

var ProgressBar = (function (Component$$1) {
  function ProgressBar () {
    Component$$1.apply(this, arguments);
  }

  if ( Component$$1 ) ProgressBar.__proto__ = Component$$1;
  ProgressBar.prototype = Object.create( Component$$1 && Component$$1.prototype );
  ProgressBar.prototype.constructor = ProgressBar;

  ProgressBar.prototype.render = function render (props) {
    var style = StyleMap.from(props.style);
    var classList = TokenList.from(props.className);
    var attrs = without(props, ['progress', 'indeterminate', 'color']);
    ...
    if (!indeterminate) { style.set('width', progress + '%'); }else { classList.add('indeterminate'); }
    if (props.color) { classList.add(props.color); }

    attrs.className = classList.toString();
    attrs.children = [preact.h('div', { className: 'ui-progress-bar__bar', style: style.toString() })];
...
var Radio = (function (Component$$1) {
  ...
  Radio.prototype.handleFocus = function handleFocus (e) {
    if (!this.state.isActive) { this.setState({ isActive: true }); }
    callHook(this.props, 'onFocus', e);
  };

  Radio.prototype.handleBlur = function handleBlur (e) {
  ...
  Radio.prototype.render = function render (props, state) {
    var this$1 = this;

    var value = props.value;
    var isChecked = value === true || value !== false && value != null && String(value).length > 0;

    var classList = TokenList.from('ui-radio');
    ...
    return preact.h(
      ...
      preact.h(
        'div',
        { className: 'ui-radio__input-wrapper' },
        preact.h('input', {
          ...
          onBlur: function (e) { return this$1.handleBlur(e); },
          onFocus: function (e) { return this$1.handleFocus(e); },
          onChange: function (e) { return this$1.handleChange(e); } }),
        preact.h('div', { className: 'ui-radio__focus-ring' }),
        preact.h('div', { className: 'ui-radio__outer-circle' }),
        preact.h('div', { className: 'ui-radio__inner-circle' })
      ),
      props.label ? preact.h(
        'div',
...
var sheetProps = ['display', 'dimmed', 'docked', 'position', 'floating', 'onShow', 'onHide', 'bodyStyle', 'bodyClassName'];
...
var Sheet = (function (Component$$1) {
  ...
  if ( Component$$1 ) Sheet.__proto__ = Component$$1;
  Sheet.prototype = Object.create( Component$$1 && Component$$1.prototype );
  Sheet.prototype.constructor = Sheet;

  Sheet.prototype.componentWillReceiveProps = function componentWillReceiveProps (props) {
    var mode = props.display ? 'in' : 'out';
    if (mode !== this.state.mode) {
      this.setState({ mode: mode });
    }
  ...
  Sheet.prototype.render = function render (props, state) {
    var onShow = function () {
    ...
    var attrs = without(props, sheetProps);
    attrs.className = classList.toString();
    attrs.name = 'ui-sheet';
    attrs.mode = state.mode;
    attrs.onClick = props.docked ? function () { return ''; } : onHide;
    attrs.onAfterEnter = onShow;
    attrs.onAfterLeave = onHide;
    attrs.children = [preact.h(
      'div',
      {
...
var Snackbar = (function (Component$$1) {
  function Snackbar(props) {
    Component$$1.call(this, props);
    this.animating = false;
  }

  if ( Component$$1 ) Snackbar.__proto__ = Component$$1;
  Snackbar.prototype = Object.create( Component$$1 && Component$$1.prototype );
  Snackbar.prototype.constructor = Snackbar;

  Snackbar.prototype.setTimeout = function setTimeout$1 (duration) {
    var this$1 = this;

    this.timer = setTimeout(function () {
      this$1.timer = null;
      callHook(this$1.props, 'onTimeout', this$1.props);
    }, duration);
  };

  Snackbar.prototype.clearTimeout = function clearTimeout$1 () {
    if (!this.timer) { return; }
    clearTimeout(this.timer);
    this.timer = null;
  };

  Snackbar.prototype.handleClick = function handleClick (e) {
    e.stopPropagation();
    callHook(this.props, 'onClick', this.props);
  };

  Snackbar.prototype.handleActionClick = function handleActionClick (e) {
    e.stopPropagation();
  ...
  Snackbar.prototype.render = function render (props) {
    var this$1 = this;

    var action = null;

    if (props.action) {
      action = preact.h(Button, {
        className: 'ui-snackbar__action-button',
        color: props.actionColor || 'primary',
        type: 'flat',
        onClick: function (e) { return this$1.handleActionClick(e); },
        value: props.action
      });
    }
    ...
    var position = props.position || 'left bottom';
    var classList = TokenList.from(position);
    ...
    var style = position.split(/\s+/).map(function (name) { return name + ':0'; }).join(';');

    var createStop = function (hook) {
      return function () {
        this$1.animating = false;
        callHook(this$1.props, hook, this$1.props);
      };
    };
    ...
    return preact.h(
      Transition,
      {
        ...
        mode: props.display ? 'in' : 'out',
        onClick: function (e) { return this$1.handleClick(e); },
        onBeforeEnter: function () { return callHook(this$1.props, 'onWillShow'); },
        onEnter: function () { return this$1.animating = true; },
        onAfterEnter: createStop('onShow'),
        onBeforeLeave: function () { return callHook(this$1.props, 'onWillHide'); },
        onLeave: function () { return this$1.animating = true; },
        onAfterLeave: createStop('onHide') },
      preact.h(
        'div',
        { className: 'ui-snackbar__message' },
        props.message
      ),
      action
    );
...
var SnackbarContainer = (function (Component$$1) {
  ...
  if ( Component$$1 ) SnackbarContainer.__proto__ = Component$$1;
  SnackbarContainer.prototype = Object.create( Component$$1 && Component$$1.prototype );
  SnackbarContainer.prototype.constructor = SnackbarContainer;

  SnackbarContainer.prototype.createSanckbar = function createSanckbar (snackbar) {
    var this$1 = this;

    snackbar = snack(snackbar, this.props.duration);
    ...
    if (!this.state.show) {
      ...
      this.awaitHide = true;
      nextTick(function () { return this$1.setState({ show: false }); });
    }
  };

  SnackbarContainer.prototype.showNextSnackbar = function showNextSnackbar () {
    ...
    var show = !!snackbar;
    this.setState({ snackbar: snackbar, show: show });

    return show;
  };

  SnackbarContainer.prototype.handleShow = function handleShow (snackbar) {
    var this$1 = this;
  ...
  SnackbarContainer.prototype.handleHide = function handleHide (snackbar) {
    var this$1 = this;

    this.awaitHide = false;
    callHook(this.props, 'onSnackbarHide', snackbar);
    callHook(snackbar, 'onHide', snackbar);
    nextTick(function () { return this$1.showNextSnackbar(); }); // 显示下一条信息
  };

  SnackbarContainer.prototype.handleClick = function handleClick (snackbar) {
    if (!this.state.show) { return; }
    this.setState({ show: false }); // 点击控制关闭
    callHook(snackbar, 'onClick', snackbar);
  };

  SnackbarContainer.prototype.handleActionClick = function handleActionClick (snackbar) {
    callHook(snackbar, 'onActionClick', snackbar);
  };

  // 超时控制关闭
  SnackbarContainer.prototype.handleTimeout = function handleTimeout () {
    this.setState({ show: false });
  };

  SnackbarContainer.prototype.render = function render (props, state) {
    var this$1 = this;

    var show = state.snackbar && state.show;
    var snackbar = state.snackbar || this.defaultSnackbar;
    if (!snackbar) { return null; }
    return preact.h(Snackbar, {
      ...
      onClick: function () { return this$1.handleClick(snackbar); },
      onActionClick: function () { return this$1.handleClick(snackbar); },
      onShow: function () { return this$1.handleShow(snackbar); },
      onHide: function () { return this$1.handleHide(snackbar); },
      onTimeout: function () { return this$1.handleTimeout(); }
    });
  };
...
exports.AppActivity = AppActivity;
...
//# sourceMappingURL=yie-common.js.map
